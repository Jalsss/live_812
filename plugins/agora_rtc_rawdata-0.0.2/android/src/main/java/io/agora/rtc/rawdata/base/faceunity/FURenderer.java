...
import io.agora.rtc.rawdata.base.faceunity.entity.FaceMakeup;
import io.agora.rtc.rawdata.base.faceunity.entity.Filter;
import io.agora.rtc.rawdata.base.faceunity.entity.MakeupItem;
import io.agora.rtc.rawdata.base.faceunity.utils.Constant;
 ...
 *
 * Faceunity Nama SDKに基づくシンプルなパッケージ、便利でシンプルな統合、理論的にシンプルな手順：
 * 1.OnEffectSelectedListenerを介したUIでの対話
 * 2.FURendererコンストラクターを合理的に呼び出す
 * 3.対応するタイミングはonSurfaceCreatedとonSurfaceDestroyedを呼び出します
 * 4.画像を処理するときにonDrawFrameを呼び出します
 */
public class FURenderer implements OnFUControlListener {
   ...
   *
   * .assetsディレクトリの下のBundleは、プログラムのデータファイルです。
   * v3.bundle：顔認識データファイルのうち、このファイルがないとシステムの初期化に失敗します。
   * face_beautification.bundle：美化と美化に関連するデータファイル。
   * anim_model.bundle：式追跡機能のためにロードする必要のあるアニメーションデータファイルを最適化します。Animojiおよびアバター機能を使用するユーザーに適しています。そうでない場合は、ロードしないでください。
   * ardata_ex.bundle：高精度モードの3Dテンソルデータファイル。 顔を変える機能に適しています。この機能を使用しない場合は、ロードする必要はありません。
   * fxaa.bundle：3D描画アンチエイリアスデータファイル。 ロード後、3Dレンダリング効果はよりスムーズになります。
   * エフェクトのカタログは、パッケージ化され署名された小道具です
   */
  // 轻美妆 bundle
  ...
  // Skin Whitening.
  private volatile static float mColorLevel = 0.0f;
  // Ruddy.
  private volatile static float mRedLevel = 0.0f;
  // Blur.
  private volatile static float mBlurLevel = 0.0f;
  private volatile static float mSkinDetect = 0.0f;
  private volatile static float mNonskinBlurScale = 0.45f;
  private volatile static float mHeavyBlur = 0.0f;
  // Eye Brighten.
  private volatile static float mEyeBright = 0.0f;
  // Tooth Whiten.
  private volatile static float mToothWhiten = 0.0f;
  // Face Outline.
  private volatile static float mFaceShape = BeautificationParams.FACE_SHAPE_CUSTOM;
  private volatile static float mFaceShapeLevel = 1.0f;
  private volatile static float mCheekThinning = 0f;
  private volatile static float mCheekV = 0.0f;
  private volatile static float mCheekNarrow = 0f;
  private volatile static float mCheekSmall = 0f;
  private volatile static float mEyeEnlarging = 0f;
  private volatile static float mIntensityForehead = 0.0f;
  private volatile static float mIntensityChin = 0.0f;
  private volatile static float mIntensityMouth = 0.0f;
  private volatile static float mIntensityNose = 0.0f;
  ...
  private boolean mIsCreateEGLContext; //是否需要手动创建EGLContext   EGLContextを手動で作成する必要がありますか
  private int mInputTextureType = 0; //输入的图像texture类型，Camera提供的默认为EXTERNAL OES   入力画像のテクスチャタイプ。Cameraが提供するデフォルトはEXTERNALOESです。
  ...
  private volatile int mInputPropOrientation = 270;//道具方向（针对全屏道具）   小道具の方向（フルスクリーンの小道具の場合）
  private volatile int mIsInputImage = 0;//输入的是否是图片    入力が画像かどうか
  private volatile int mCurrentCameraType = CAMERA_FACING_FRONT;
  private volatile int mMaxFaces = 4; //同时识别的最大人脸    同時に認識される最大の顔
   ...
   * faceunityの対応するリソースを作成して初期化します
   */
  @RequiresApi(api = Build.VERSION_CODES.JELLY_BEAN_MR2)
  public void onSurfaceCreated() {
    mCurrentThreadID = Thread.currentThread().getId();
    Log.d(TAG, "onSurfaceCreated(" + mCurrentThreadID + ")");
     ...
     *
     * fuCreateEGLContextはOpenGL環境を作成します
     * OpenGL環境がない場合の呼び出しに適しています
     * fuCreateEGLContextが呼び出された場合、破棄するときにfuReleaseEGLContextを呼び出す必要があります
     */
  ...
  /**
   * 获取faceunity sdk 版本库    sdk version
   */
  ...
  public int onDrawRgbaFrame(byte[] img, int w, int h) {
    ...
    if (mCurrentCameraType != CAMERA_FACING_FRONT)
   ...
   * @return
   */
  public int onDrawFrame(byte[] y_buffer, byte[] u_buffer, byte[] v_buffer, int y_stride, int u_stride, int v_stride, int w, int h) {
    ...
    //Log.d(TAG,"faceunity.fuRenderToYUVImage Begin(" + threadID + ")");
    int fuTex = faceunity.fuRenderToYUVImage(y_buffer,u_buffer, v_buffer, y_stride, u_stride, v_stride, w, h, mFrameId++, mItemsArray, flags);
    //Log.d(TAG,"faceunity.fuRenderToYUVImage End(" + threadID + ")");
    //Log.d(TAG,"faceunity.fuRenderToYUVImage(" + threadID + ") : " + fuTex + ", faceunity.fuIsTracking(): " + faceunity.fuIsTracking());
  ...
  /*  さまざまなテクスチャタイプと入力方向に応じてrotationModeを変更します */
  private int createRotationMode() {

    int rotMode = faceunity.FU_ROTATION_MODE_90;
    int deviceOrientation = mDeviceOrientation;
    int cameraType = mCurrentCameraType;
   ...
   * 対応する基になるデータパッケージをグローバルにロードし、アプリケーションの使用中に一度だけ初期化する必要があります
   * システム環境を初期化し、システムデータをロードし、ネットワーク認証を実行します。 他のSDKインターフェイスを呼び出す前に実行する必要があります。そうしないと、クラッシュが発生します。
   */
   ...
   *  アセットフォルダまたはローカルディスクからファイルを読み取ります
   * @param context
   * @param path
   * @return
   */
  ...
  private void prepareDrawFrame() {
    ...
    //获取人脸是否识别，并调用回调接口
    int isTracking = faceunity.fuIsTracking();
    //Log.d(TAG,"faceunity.fuIsTracking() : " + isTracking);
  ...
  private void destroyItem(int index) {
    if (mItemsArray[index] != 0) {
      faceunity.fuDestroyItem(mItemsArray[index]);
    }
    mItemsArray[index] = 0;
  }
  ...
  public void onEffectSelected(Effect effectItemName) {
    if (effectItemName.path().length()<=0){
      mItemsArray[1] = 0;
    }
  ...
  @Override
  public void onEnableFaceBeautification() {
    queueEvent(new Runnable() {
      @Override
      public void run() {
        mFuItemHandler.removeMessages(ITEM_ARRAYS_FACE_BEAUTY_INDEX);
        mFuItemHandler.sendEmptyMessage(ITEM_ARRAYS_FACE_BEAUTY_INDEX);
      }
    });
}
  ...
  @Override
  public void onDisableFaceBeautification() {
    destroyItem(ITEM_ARRAYS_FACE_BEAUTY_INDEX);
    isNeedUpdateFaceBeauty = false;
  }

  @Override
  public void onFilterSelected(String filterName, Double filterLevel) {
    final String name = filterName;
    final float level = filterLevel.floatValue();
    queueEvent(new Runnable() {
      @Override
      public void run() {
        faceunity.fuItemSetParam(mItemsArray[ITEM_ARRAYS_FACE_BEAUTY_INDEX], BeautificationParams.FILTER_NAME, name);
        faceunity.fuItemSetParam(mItemsArray[ITEM_ARRAYS_FACE_BEAUTY_INDEX], BeautificationParams.FILTER_LEVEL, level);
      }
    });
  }

  @Override
  public void onSkinWhiteningSelected(Double colorLevel) {
    final float level = colorLevel.floatValue();
    queueEvent(new Runnable() {
      @Override
      public void run() {
        faceunity.fuItemSetParam(mItemsArray[ITEM_ARRAYS_FACE_BEAUTY_INDEX], BeautificationParams.COLOR_LEVEL, level);
      }
    });
  }

  @Override
  public void onRuddySelected(Double redLevel) {
    final float level = redLevel.floatValue();
    queueEvent(new Runnable() {
      @Override
      public void run() {
        faceunity.fuItemSetParam(mItemsArray[ITEM_ARRAYS_FACE_BEAUTY_INDEX], BeautificationParams.RED_LEVEL, level);
      }
    });
  }

  @Override
  public void onBlurSelected(Double blurLevel, Integer skinDetect, Double nonskinBlurScale, Integer heavyBlur) {
    final float bl = blurLevel.floatValue();
    final int sd = skinDetect.intValue();
    final float nbs = nonskinBlurScale.floatValue();
    final float hb = heavyBlur.floatValue();
    queueEvent(new Runnable() {
      @Override
      public void run() {
        faceunity.fuItemSetParam(mItemsArray[ITEM_ARRAYS_FACE_BEAUTY_INDEX], BeautificationParams.BLUR_LEVEL, bl);
        faceunity.fuItemSetParam(mItemsArray[ITEM_ARRAYS_FACE_BEAUTY_INDEX], BeautificationParams.SKIN_DETECT, sd);
        faceunity.fuItemSetParam(mItemsArray[ITEM_ARRAYS_FACE_BEAUTY_INDEX], BeautificationParams.NONSKIN_BLUR_SCALE, nbs);
        faceunity.fuItemSetParam(mItemsArray[ITEM_ARRAYS_FACE_BEAUTY_INDEX], BeautificationParams.HEAVY_BLUR, hb);
      }
    });
  }

  @Override
  public void onEyeBrightenSelected(Double eyeBright) {
    final float eb = eyeBright.floatValue();
    queueEvent(new Runnable() {
      @Override
      public void run() {
        faceunity.fuItemSetParam(mItemsArray[ITEM_ARRAYS_FACE_BEAUTY_INDEX], BeautificationParams.EYE_BRIGHT, eb);
      }
    });
  }

  @Override
  public void onToothWhitenSelected(Double toothWhiten) {
    final float tw = toothWhiten.floatValue();
    queueEvent(new Runnable() {
      @Override
      public void run() {
        faceunity.fuItemSetParam(mItemsArray[ITEM_ARRAYS_FACE_BEAUTY_INDEX], BeautificationParams.TOOTH_WHITEN, tw);
      }
    });
  }

  @Override
  public void onFaceOutlineSelected(Integer faceShape, Double faceShapeLevel, Double eyeEnlarging,
          Double cheekThinning, Double intensityForehead, Double intensityChin, Double intensityNose,
          Double intensityMouth) {
    final int fs = faceShape.intValue();
    final float fsl = faceShapeLevel.floatValue();
    final float ee = eyeEnlarging.floatValue();
    final float ct = cheekThinning.floatValue();
    final float iF = intensityForehead.floatValue();
    final float iC = intensityChin.floatValue();
    final float iN = intensityNose.floatValue();
    final float iM = intensityMouth.floatValue();
    queueEvent(new Runnable() {
      @Override
      public void run() {
        faceunity.fuItemSetParam(mItemsArray[ITEM_ARRAYS_FACE_BEAUTY_INDEX], BeautificationParams.FACE_SHAPE, fs);
        faceunity.fuItemSetParam(mItemsArray[ITEM_ARRAYS_FACE_BEAUTY_INDEX], BeautificationParams.FACE_SHAPE_LEVEL, fsl);
        faceunity.fuItemSetParam(mItemsArray[ITEM_ARRAYS_FACE_BEAUTY_INDEX], BeautificationParams.EYE_ENLARGING, ee);
        faceunity.fuItemSetParam(mItemsArray[ITEM_ARRAYS_FACE_BEAUTY_INDEX], BeautificationParams.CHEEK_THINNING, ct);
        faceunity.fuItemSetParam(mItemsArray[ITEM_ARRAYS_FACE_BEAUTY_INDEX], BeautificationParams.INTENSITY_FOREHEAD, iF);
        faceunity.fuItemSetParam(mItemsArray[ITEM_ARRAYS_FACE_BEAUTY_INDEX], BeautificationParams.INTENSITY_CHIN, iC);
        faceunity.fuItemSetParam(mItemsArray[ITEM_ARRAYS_FACE_BEAUTY_INDEX], BeautificationParams.INTENSITY_NOSE, iN);
        faceunity.fuItemSetParam(mItemsArray[ITEM_ARRAYS_FACE_BEAUTY_INDEX], BeautificationParams.INTENSITY_MOUTH, iM);
      }
    });
  }
   ...
   *  fuCreateItemFromPackageはアイテムをロードします
...
//        }
    int back = mCurrentCameraType == CAMERA_FACING_BACK ? 1 : 0;
  ...
  public static class Builder {
    ...
    private boolean isNeedFaceBeauty = true;
    private int currentCameraType = CAMERA_FACING_FRONT;
