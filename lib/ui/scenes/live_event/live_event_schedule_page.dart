...
class LiveEventSchedulePage extends StatefulWidget {
  @override
  _LiveEventSchedulePageState createState() => _LiveEventSchedulePageState();
}

class _LiveEventSchedulePageState extends State<LiveEventSchedulePage> {
  ...
  void initState() {
    ...
    Future(() {
      if (mounted) {
        _requestScheduleLiveEvent(context);
      }
    });
  }
  ...
  /// イベント一覧の取得.
  Future<void> _requestScheduleLiveEvent(BuildContext context) async {
    try {
      _liveEventList = await LiveEventUseCase.requestScheduleLiveEvent(context);
      if (mounted) {
        setState(() {});
      }
  ...
  Future _onTapLiveEvent({BuildContext context, LiveEvent liveEvent}) async {
    ...
    if (refresh == true) {
      await _requestScheduleLiveEvent(context);
    }
  }
