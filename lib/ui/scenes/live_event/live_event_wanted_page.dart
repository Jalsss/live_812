...
class LiveEventWantedPage extends StatefulWidget {
  @override
  _LiveEventWantedPageState createState() => _LiveEventWantedPageState();
}

class _LiveEventWantedPageState extends State<LiveEventWantedPage> {
  ...
  void initState() {
    ...
    Future(() {
      if (mounted) {
        _requestRecruitingLiveEvent(context);
      }
    });
  }
  ...
  /// イベント一覧の取得.
  Future<void> _requestRecruitingLiveEvent(BuildContext context) async {
    try {
      _liveEventList = await LiveEventUseCase.requestWantedLiveEvent(context);
      if (mounted) {
        setState(() {});
      }
  ...
  Future _onTapLiveEvent({BuildContext context, LiveEvent liveEvent}) async {
    ...
    // 画面更新.
    if (refresh == true) {
      await _requestRecruitingLiveEvent(context);
    }
  }
