...
class LiveViewMessageWidget extends StatefulWidget {
  ...
  @override
  _LiveViewMessageWidgetState createState() => _LiveViewMessageWidgetState();
}

class _LiveViewMessageWidgetState extends State<LiveViewMessageWidget> {
      ...
      NotificationListener(
          ...
          child: SingleChildScrollView(
            ...
            child: Container(
              ...
              child: Opacity(
                  opacity: widget.stateView == 0 || widget.stateView == 1 ? 1 : 0,
                  child: Column(
        ...
        Opacity(
          opacity: widget.stateView == 0 || widget.stateView == 1 ? (showButton ? 1 : 0) :0,
          child: GestureDetector(
            child: Container(
              padding: EdgeInsets.all(3),
              constraints: BoxConstraints(
                  maxWidth: 36, maxHeight: 36, minWidth: 26, minHeight: 26),
  ...
  Widget viewMessage(MessageModel message) {
    return Container(
                  ...
                  child: RawMaterialButton(
                    ...
                    child: CircleAvatar(
                      ...
                      backgroundImage: SafeNetworkImage(
                          BackendService.getUserThumbnailUrl(message.userId,
                              small: true)),
                    ...
                    onPressed: () {
                      showDialog(
                        context: context,
                        builder: (context) =>
                            ProfileDialog(userId: message.userId),
                      );
                    },
                  ),
                ),
  ...
  void _receiveLike(dynamic data) {
    ...
    if (userModel.id == like.userId) {
      ...
      _addMessage(MessageModel(MessageType.LIKE,
          [HighLight(like.nickName), 'がいいね💗️'], like.userId));
    }
    if(scrollController.position.pixels < scrollController.position.maxScrollExtent) {
      ...
      if (userModel.id == like.userId) {
        ...
        _addMessageBackup(MessageModel(MessageType.LIKE,
            [HighLight(like.nickName), 'がいいね💗️'], like.userId));
      }
      setState(() {
        showButton = true;
  ...
  void _receiveGift(dynamic data) {
    ...
    if (widget.giftInfoList != null) {
      final giftInfo =
          widget.giftInfoList.indexWhere((g) => g.id.toString() == gift.id);
      if (giftInfo != null) {
        giftName = widget.giftInfoList[giftInfo].name;
      }
    }
